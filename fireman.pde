/*
 ...
 Version 1.21 5-23-2010
 Andrew Macrae, drewmacrae@gmail.com
 Thanks to Mark White for version 1.2
 ...
 The three menu game options are:
 
 //1. Sound (not too obnoxious)
 //Removed by Drew Macrae, meggy sketches can be muted by pressing any button during startup
 1. Fire (slightly distracting)
 2. Animate the babies as they bounce (definitely distracting)
 3. Show background graphics (extremely distracting)
...
namespace fireman//the fireman
{
  byte Path[59][6]={// creates the path for the babies to bounce 
  //across the screen + the animation sequence for each babie
    {0,7,0,7,1,7}
   ,{0,7,0,7,1,7}
   ,{0,7,0,7,1,7}
   ,{0,7,0,7,1,7}
  ...
  byte ManX[4] = {
    0,0,3,6  };
  byte Fire[11] = {
    DimRed, Red, DimRed, Yellow, Dark, Dark, Red, Dark, Orange, Orange, Red  };

  void sub_setup() // Set initial values
  {
  ...
  void DrawFire()
  {
    ...
    kk = random(0,5); 
    ll = random(0,5);
    DrawPx(ii,7-jj,Fire[l]); 
    DrawPx(kk,7,Fire[k]);// DrawPx(ll,7-ii,Fire[j]); DrawPx(jj,7-kk,Fire[i]);
  }//end DrawFire
  ...
  void DrawBackground()
  {
    ...
    k=random(2,8);
    //Ambulance
    DrawPx(4,4,CustomColor1);
    DrawPx(4,3,CustomColor1);
    DrawPx(4,2,CustomColor1);
    DrawPx(4,1,CustomColor1);
    DrawPx(5,4,CustomColor1);
    DrawPx(5,3,CustomColor1);
    DrawPx(5,2,CustomColor6);
    DrawPx(5,1,CustomColor1);
    DrawPx(6,4,CustomColor1);
    DrawPx(6,3,CustomColor6);
    DrawPx(6,2,CustomColor6);
    DrawPx(6,1,CustomColor6);
    DrawPx(7,4,CustomColor1);
    DrawPx(7,3,CustomColor1);
    DrawPx(7,2,CustomColor6);
    DrawPx(7,1,CustomColor1);
    //Building
    for (i=1;i<8;i++) {
      DrawPx(0,i,CustomColor1); 
      DrawPx(1,i,CustomColor1);
    }
    //Grass
    for (i=0;i<8;i++) {
      DrawPx(i,0,Green);
  ...
  void DrawFiremen()
  {
    if (HandsUp){
      if (ManSpot>1) {
        ...
        DrawPx(ManX[ManSpot]-1,1,CustomColor8);
        DrawPx(ManX[ManSpot]-2,0,CustomColor3); 
        DrawPx(ManX[ManSpot]-1,0,CustomColor4);
      }
      if (ManSpot<3) {
        DrawPx(ManX[ManSpot]+2,2,CustomColor5);
        DrawPx(ManX[ManSpot]+2,1,CustomColor8);
      ...
      if (ManSpot>1) {
        DrawPx(ManX[ManSpot]-2,2,CustomColor5);
        DrawPx(ManX[ManSpot]-2,1,CustomColor8); 
        DrawPx(ManX[ManSpot]-1,1,CustomColor3);
        DrawPx(ManX[ManSpot]-2,0,CustomColor3); 
        DrawPx(ManX[ManSpot]-1,0,CustomColor8);
      }
      if (ManSpot<3) {
        DrawPx(ManX[ManSpot]+2,2,CustomColor5);
        DrawPx(ManX[ManSpot]+2,1,CustomColor8);
  ...
  {
    byte i;
    DrawPx(5,7,CustomColor7);
    DrawPx(6,7,CustomColor7);
    DrawPx(7,7,CustomColor7);
    for (i=0;i<DeadBabies;i++){
      DrawPx((7-i),7,DimBlue);
    }
    SetAuxLEDs(Score);
    
    if(Score+1>EEPROM.read(FIREMAN)+1)
      EEPROM.write(FIREMAN,Score);
  ...
  void CheckBabies()
  {
    ...
    for(i=BabyNum;i>0;i--){
      ...
      if (Baby[i][0] < 3) {
        BirthControl = 1;
      }// don't drop a baby if there is no chance to catch it    
      if ((Baby[i][0] < 26) && (Baby[i][0] > 14)) {
        BirthControl = 1;
      }    
      if ((Baby[i][0] < 44) && (Baby[i][0] > 36)) {
        BirthControl = 1;
      }    
      switch (Baby[i][0]) {
        //First Fall  
  ...
  void LaborCheck()// make new babies
  {
    ...
    if (!BirthControl && (OneNightStand > Fertility)) {
      NewBaby=1;
    }
    if (NewBaby) {
      BabyNum++;
  ...
  void DeadBaby()//oops
  {
    ...
    if (SplatShow > 4){
      DrawPx(ManX[BabySplat], 0, Violet); 
      Tone_Start(12000+(SplatShow * 200),60); 
      if (SplatShow > 10){
        SplatShow = 0;
      }
    }
  ...
  void DrawMenu()
  {
    ...
    byte MenuSound[7] = {
      Dark, Dark, DimGreen, DimGreen, DimGreen, Green, FullOn    };
    ...
    for (i=0;i<8;i++){
      DrawPx(i,3,DimViolet); 
      DrawPx(i,4,DimViolet); 
      DrawPx(3,i,DimViolet); 
      DrawPx(4,i,DimViolet);
    }
    switch(ThisMenu){
    ...
    case 2:// sound
      ...
      if ((MenuButton == 1) || (MenuButton == 2)) {
        Meg.SoundAllowed = !Meg.SoundAllowed;
      } */
      break;
    case 3:// background
      for (i=0;i<4;i++){
    ...
    if (Meg.SoundAllowed) {
      ...
      DrawPx(6,2,MenuSound[j]); 
      DrawPx(5,1,MenuSound[j]); 
      DrawPx(6,1,MenuSound[j]);
      DrawPx(7,2,MenuSound[k]); 
      DrawPx(7,1,MenuSound[k]); 
      DrawPx(7,0,MenuSound[k]); 
      DrawPx(5,0,MenuSound[k]); 
      DrawPx(6,0,MenuSound[k]);
    }//end BurnBaby
    if (DrawBack) {
      DrawPx(0,0,Green); 
      DrawPx(1,0,Green); 
      DrawPx(2,0,Green); 
      DrawPx(0,1,CustomColor1); 
      DrawPx(1,1,CustomColor1); 
      DrawPx(0,2,CustomColor1); 
      DrawPx(1,2,CustomColor1);
    }
    MenuButton = 0;
    DisplaySlate();
  }

  void sub_loop()  // Run main loop
  {
    byte i,j,k;
  ...
  void splash()//the intro screen will work for the menu
  {
    IntroScreen();
  }
}
